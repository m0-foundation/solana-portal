...
impl ProposeAdmin<'_> {
    pub fn handler(ctx: Context<Self>, new_admin: Pubkey) -> Result<()> {
        ctx.accounts.wormhole_global.pending_admin = Some(new_admin);
...
pub struct AcceptAdmin<'info> {
    ...
    #[account(
        ...
        seeds = [GLOBAL_SEED],
        bump = wormhole_global.bump,
        constraint = wormhole_global.pending_admin == Some(pending_admin.key()) @ BridgeError::NotAuthorized,
    )]
    pub wormhole_global: Account<'info, WormholeGlobal>,
}
...
impl AcceptAdmin<'_> {
    pub fn handler(ctx: Context<Self>) -> Result<()> {
        let old_admin = ctx.accounts.wormhole_global.admin;
        let new_admin = ctx.accounts.pending_admin.key();

        ctx.accounts.wormhole_global.admin = new_admin;
        ctx.accounts.wormhole_global.pending_admin = None;
