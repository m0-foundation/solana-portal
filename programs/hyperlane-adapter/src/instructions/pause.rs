...
impl ManagePause<'_> {
    pub fn handler(
        ...
        if let Some(outgoing_paused) = outgoing_paused {
            ctx.accounts.hyperlane_global.outgoing_paused = outgoing_paused;
        }
        if let Some(incoming_paused) = incoming_paused {
            ctx.accounts.hyperlane_global.incoming_paused = incoming_paused;
        }

        msg!(
            "Hyperlane pause updated: outgoing_paused={}, incoming_paused={}",
            ctx.accounts.hyperlane_global.outgoing_paused,
            ctx.accounts.hyperlane_global.incoming_paused
        );
